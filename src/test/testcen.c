...
bool setup()
{
    ...
    // Initialize video
    if ( !setupVideo("/dev/video0", VFRAME_WIDTH, VFRAME_HEIGHT, &pongInstance.cameraInfo))
    {
        loge( "Failed to setup video A" );
        return false;
    }

//    if ( !setupVideo("/dev/video1", VFRAME_WIDTH, VFRAME_HEIGHT, &videoReturnValue_B))
//    {
//        loge( "Failed to setup video A" );
//        return false;
//    }
...
bool start()
{
    ...
    if ( !startGreenFilter() )  return false;
    if ( !startPaddleTracker()) return false;
    if ( !startActuator() )     return false;
