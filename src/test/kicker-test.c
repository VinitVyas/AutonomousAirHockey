...
int main( int argc, char *argv[] )
{
    ...
    setupIO( &pongInstances[0] );
    setupIO( &pongInstances[1] );

    keepWorking = true;
    startKicker( &pongInstances[0] );
    startKicker( &pongInstances[1] );

    getchar();
    keepWorking = false;

    if ( pongInstances[0].kickerThreadId!= 0 )      pthread_join(  pongInstances[0].kickerThreadId, NULL );
    if ( pongInstances[1].kickerThreadId!= 0 )      pthread_join(  pongInstances[1].kickerThreadId, NULL );
    ...
    switch ( step )
    {
        case 0:
            ...
            if ( span++ > 1 )
            {
                ...
                gpioWritePin( STEP, toogle ? HIGH : LOW );
                span = 0;
            }
