...
bool filterRedFrame(unsigned char *fullImg, unsigned char *filteredImg, int filterSize) //Red is puck
{
    ...
    int i;
//    unsigned char header[16] = "P6\n320 215 255\n"; //header to be modified for different sizes
//    FILE *fout0, *fout1;
    //int flag = 0;

//    if(pongInstance->id == 0)
//    {
//        fout0 = fopen("filterRedPuck0.ppm", "w"); //writing to video0 file
//        fwrite(header, 1, 15, fout0);
//        flag = 1;
//    }
//
//    if(pongInstance->id == 1)
//    {
//        fout1 = fopen("filterRedPuck1.ppm", "w"); //writing to video1 file
//        fwrite(header, 1, 15, fout1);
//    }
    ...
    for(i=VIDEO_DIVIDE; i<filterSize;++i) //Skip G and B
    {
        ...
        /*if(flag == 1)
        {
            fwrite((void *)&filteredImg[i], 1, 1, fout0);
            fwrite((void *)&filteredImg[i], 1, 1, fout0);
            fwrite((void *)&filteredImg[i], 1, 1, fout0);
        }
        else
        {
            fwrite((void *)&filteredImg[i], 1, 1, fout1);
            fwrite((void *)&filteredImg[i], 1, 1, fout1);
            fwrite((void *)&filteredImg[i], 1, 1, fout1);
        }*/
...
//    if(pongInstance->id == 0)
//    {
//        fout0 = fopen("filterGreenPaddle0.ppm", "w"); //writing to video0 file
//        fwrite(header, 1, 15, fout0);
//    }
//
//    if(pongInstance->id == 1)
//    {
//        fout1 = fopen("filterGreenPaddle1.ppm", "w"); //writing to video1 file
//        fwrite(header, 1, 15, fout1);
//    }
...
int paddleCentroid(unsigned char *img, int wd, int ht, int pongId)
{
    ...
    //inTarget=0;
    //xCnt=0, xCntPrev=0, xCntPrevPrev=0, xCntSum=0, xEdgeL=0, xEdgeR=0, xCent=0, xCntMax=0;

    //printf("Paddle - xCentFinal = %d", xCentFinal);
    //printf("Position of the paddle: %d\n", xCentFinal);
